...
void GUIText::step(string_view, bool, uint8_t *buffer, size_t buffer_size, handler::Step& out) {
    ...
    if (!headers_sent) {
        out.written = write_headers(buffer, buffer_size, Status::Ok, ContentType::ApplicationOctetStream, handling, std::nullopt, std::nullopt);
